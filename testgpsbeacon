...
def reverse_geocode(lat, lng):
    try:
        ...
        # Try to find result matching preferred types
        for pref_type in preferred_types:
            for result in r["results"]:
                ...
                if pref_type in result["types"]:
                    # Get the formatted address without plus code
                    addr = result["formatted_address"]
                    
                    # Remove plus code if it appears in the address
                    if "+" in addr:
                        # Split by comma and filter out plus code parts
                        parts = [p.strip() for p in addr.split(",")]
                        parts = [p for p in parts if "+" not in p]
                        addr = ", ".join(parts)
                    
                    loc = result["geometry"]["location"]
                    return addr, loc["lat"], loc["lng"], "reverse"
